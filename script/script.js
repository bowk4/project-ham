...
// Get the number of cards that can fit in the carousel at once
    ...
    // ================================ //
    const grid = document.querySelector(".gallery-masonry-container")
    const masonry = new Masonry (grid, {
        itemSelector: ".grid-item",
        gutter: 10,
    })
    
    const loadMoreButton = document.getElementById("showImgButton6");
    const hiddenImages = Array.from(grid.querySelectorAll(".grid-item.hidden"));

    loadMoreButton.addEventListener("click", function () {
        loadMoreButton.classList.add("loading"); // Додаємо клас "loading" до кнопки
